...
inc(n);
StatusBar1.Panels[2].text:='Промахов: '+IntToStr(n);
EndGame();
end;
end;
...
begin
 if (t>0) and (RunGame=True) then begin
inc(p);
StatusBar1.Panels[1].text:='Попаданий: '+IntToStr(p);
EndGame();
end;
end;
...
StatusBar1.Panels[0].text:='Уровень: '+IntToStr(Level);
StatusBar1.Panels[1].text:='Попаданий: '+IntToStr(p);
StatusBar1.Panels[2].text:='Промахов: '+IntToStr(n);
StatusBar1.Panels[3].text:='Время: '+IntToStr(t);
